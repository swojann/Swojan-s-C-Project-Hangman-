...
int playGame(Word words[MAX_WORDS]) {
    srand(time(NULL)); // select random word
    int wins = 0;
    ...
    for (int i = 0; i < MAX_WORDS; ++i) {
        ...
        currentWord->used = 1;
        int lives = MAX_LIVES;
        char guessed[strlen(currentWord->word) * 2]; // Adjusting spaces between underscores
        ...
        guessed[strlen(currentWord->word) * 2 - 1] = '\0'; 
        guessCount = 0;
    ...
    return wins == MAX_WORDS; // If all words are guessed correctly then return 1 otherwise 0
}
